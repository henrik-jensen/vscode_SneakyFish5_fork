...
export class EditorStatus extends Disposable implements IWorkbenchContribution {
	...
	private updateStatusBar(): void {
		...
		if (activeCodeEditor) {
			...
			const onReadOnlyModeConfigChange = Event.filter(this.configurationService.onDidChangeConfiguration, e => e.affectsConfiguration('editor.readOnly'));
			this._register(onReadOnlyModeConfigChange(() => this.onReadOnlyModeChange(activeCodeEditor)));
	...
	private onReadOnlyModeChange(editorWidget: ICodeEditor | undefined): void {
		...
		if (editorWidget) {
			readOnlyMode = this.configurationService.getValue<boolean>('editor.readOnly');
